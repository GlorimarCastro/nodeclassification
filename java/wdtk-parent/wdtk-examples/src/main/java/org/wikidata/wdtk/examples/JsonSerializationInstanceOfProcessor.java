...
public class JsonSerializationInstanceOfProcessor implements EntityDocumentProcessor{
	...
	//--------------------------------------------------
	/**
	 * Creates a new JsonSerializationInstanceOfProcessor object. The properties filter are not
	 * set  and the default language is English. Also the default value for revisions
	 * are set to get the current revisions. 
	 * @throws IOException 
	 */
	private  JsonSerializationInstanceOfProcessor(String instanceStr) throws IOException {
		propertyClass 		= instanceStr;
	 ...
	 * @throws IOException 
	 */
	public static JsonSerializationInstanceOfProcessor getInstanceOf(String instanceStr) throws IOException{
		if(jsonProcessor == null){
			return new JsonSerializationInstanceOfProcessor(instanceStr);
		}
			...
			for (Statement s : sg.getStatements()) {
				if (s.getClaim().getMainSnak() instanceof ValueSnak) {
					...
					if (v instanceof ItemIdValue
							&& propertyClass.equals(((ItemIdValue) v).getId())) {
