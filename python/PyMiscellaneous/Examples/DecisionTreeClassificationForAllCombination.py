...
outputDirectory = "D:\\CrossValidationSamples\\CrossGraph\\DecisionTree\\try\\"

f_colm = [6, 11, 10, 33,34,35,36]#range(1,41) #features to extrac from files
pk_colm = 0
class_clmn = 40
...
for k in range(start_k,f_amnt + 1):
    ...
    for k_comb_i in k_comb: #this is one system
        ...
        for fold_x in range(0, fold_amnt):
            ...
            foldX_f_training    = f_training[str(fold_x)]
            foldX_cass_training = class_training[str(fold_x)]
            foldX_f_testing     = f_testing[str(fold_x)]
            foldX_pk            = pk[str(fold_x)]
            foldX_truth         = truth[str(fold_x)]
            ...
            #print(foldX_f_training)
            #print(foldX_cass_training)
